-- pomocné selekty
select * from covid19_basic_differences ; -- where country = "czechia";
select * from countries; -- where country = "czech republic";
select * from covid19_tests ct; -- where iso = "CZE";
...
select DISTINCT religion from religions order by religion;
select * from weather;
...
select cov.`date`, case when WEEKDAY(cov.`date`) in (5,6) then 1 else 0 end as is_weekend, 
    ...
    cov.country, cov.confirmed, tes.tests_performed, loo.population,
...
left join covid19_tests as tes 
    on cov.`date` = tes.`date` and loo.iso3 = tes.iso
